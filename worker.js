                    ...
                    if (url.searchParams.get('token') === 订阅TOKEN) {
                        ...
                        if (!跳过mihomo缓存) {
                            try {
                                ...
                                if (cachedMihomo) return cachedMihomo;
                            } catch (error) {
                                console.error(`读取mihomo响应缓存失败: ${error.message}`);
                            }
                        }
                        config_JSON = await 读取config_JSON(env, host, userID, env.PATH);
                        ...
                        const mihomoProvider = 生成MihomoProvider(完整优选IP, config_JSON, host);
                        const mihomo响应 = new Response(mihomoProvider, {
                            status: 200,
                            headers: {
                                "content-type": "application/x-yaml; charset=utf-8",
                                "Cache-Control": `public, max-age=${MIHOMO优选缓存秒}, s-maxage=${MIHOMO优选缓存秒}`,
                                "Profile-Update-Interval": config_JSON.优选订阅生成.SUBUpdateTime || 1,
                            }
                        });
                        if (!跳过mihomo缓存) ctx.waitUntil(caches.default.put(mihomo响应缓存KEY, mihomo响应.clone()).catch(error => console.error(`写入mihomo响应缓存失败: ${error.message}`)));
                        return mihomo响应;
                    }
...
async function MD5MD5(文本) {
    ...
    let 第一次哈希;
    try {
        第一次哈希 = await crypto.subtle.digest(算法, 编码器.encode(文本));
    } catch {
        算法 = 'SHA-256';
        第一次哈希 = await crypto.subtle.digest(算法, 编码器.encode(文本));
    }
    ...
    const 第二次哈希 = await crypto.subtle.digest(算法, 编码器.encode(第一次十六进制.slice(7, 27)));
    const 第二次哈希数组 = Array.from(new Uint8Array(第二次哈希));
    const 第二次十六进制 = 第二次哈希数组.map(字节 => 字节.toString(16).padStart(2, '0')).join('');

    return 第二次十六进制.toLowerCase().slice(0, 32);
}
...
async function 生成随机IP(request, count = 16, 指定端口 = -1, 备注前缀 = '') {
    ...
    const 打散CIDR = [...cidrList].sort(() => Math.random() - 0.5);
    const randomIPs = Array.from({ length: count }, (_, index) => {
        const ip = generateRandomIPFromCIDR(打散CIDR[index % 打散CIDR.length]);
        const remark = 备注前缀 || cfname;
        return `${ip}:${指定端口 === -1 ? cfport[Math.floor(Math.random() * cfport.length)] : 指定端口}#${remark}`;
    });
    return [randomIPs, randomIPs.join('\n')];
}

const MIHOMO优选源列表 = [
    { name: 'qilin-bestcf', url: 'https://bestcf.pages.dev/uouin/all.txt', parser: 'api', limit: 80 },
    { name: '090227-bestcf-all', url: 'https://090227.pages.dev/bestcf?isp=all&ips=20', parser: 'api', limit: 80 },
    { name: 'wetest-cloudflare-v4', url: 'https://www.wetest.vip/page/cloudflare/address_v4.html?max=200', parser: 'html', limit: 80 },
    { name: 'IPDB-bestcf', url: 'https://ipdb.api.030101.xyz/?type=bestcf', parser: 'api', limit: 50 },
    { name: 'addressesapi-cloudflare-yes', url: 'https://addressesapi.090227.xyz/CloudFlareYes', parser: 'api', limit: 50 },
    { name: 'addressesapi-ip164746', url: 'https://addressesapi.090227.xyz/ip.164746.xyz', parser: 'api', limit: 50 },
    { name: 'cf-speed-dns-top', url: 'https://ip.164746.xyz/ipTop.html', parser: 'api', limit: 50 },
    { name: 'cf-speed-dns-top10', url: 'https://ip.164746.xyz/ipTop10.html', parser: 'api', limit: 50 },
    { name: 'cf-090227-page', url: 'https://cf.090227.xyz/', parser: 'html', limit: 30 },
    { name: 'cf-speed-dns-page', url: 'https://ip.164746.xyz/', parser: 'html', limit: 30 },
    { name: 'moist-bestcf', url: 'https://bestcf.pages.dev/moistr/all.txt', parser: 'api', limit: 20 },
...
const MIHOMO优选域名列表 = [
    ...
    { name: 'domain-cf-877774', hostname: 'cf.877774.xyz' },
    { name: 'domain-cf-182682', hostname: 'cf.cloudflare.182682.xyz' },
    { name: 'domain-cf-tencentapp', hostname: 'cf.tencentapp.cn' },
    { name: 'domain-saas-sin-fan', hostname: 'saas.sin.fan' },
    { name: 'domain-visa-cn', hostname: 'www.visa.cn' },
    { name: 'domain-shopify', hostname: 'www.shopify.com' },
    { name: 'domain-mfa-gov-ua', hostname: 'mfa.gov.ua' },
    { name: 'domain-nexusmods', hostname: 'staticdelivery.nexusmods.com' },
];
const MIHOMO优选缓存KEY = 'mihomo_preferred_cf_ips_v1';
const MIHOMO优选缓存秒 = 300;
const MIHOMO自定义优选单源上限 = 300;
...
async function 获取Mihomo优选IP(env, request, url, config_JSON) {
    const 请求数量 = url.searchParams.has('count') ? parseInt(url.searchParams.get('count'), 10) : null;
    const 数量上限 = Number.isFinite(请求数量) && 请求数量 > 0 ? 请求数量 : null;
    ...
    if (!跳过优选缓存) {
        try {
            ...
            console.error(`读取mihomo优选热缓存失败: ${error.message}`);
        }
    }
...
async function 补足Mihomo优选IP(list, targetCount, request) {
    ...
    for (let i = 0; merged.length < targetCount && i < 4; i++) {
        ...
        merged = 去重Mihomo优选IP(merged.concat(randomIPs));
    }
    return 限制Mihomo数量(merged, targetCount);
}

async function 读取Mihomo自定义优选源(env) {
    const results = [];
    try {
        if (!env.KV || typeof env.KV.get !== 'function') return results;
        const text = await env.KV.get('ADD.txt');
        if (!text) return results;
        results.push(...解析Mihomo优选文本(text, 'ADD.txt'));
        const urls = [...new Set(String(text).match(/https?:\/\/[^\s,'"]+/gi) || [])];
        if (urls.length) {
            const settled = await Promise.allSettled(urls.map(async (sourceURL) => {
                const sourceResults = [];
                const [优选API的IP] = await 请求优选API([sourceURL], '443', 3000);
                for (const item of 优选API的IP) {
                    const parsed = 标准化Mihomo优选IP(item, 'ADDAPI');
                    if (parsed) sourceResults.push(parsed);
                }
                return 限制Mihomo数量(去重Mihomo优选IP(sourceResults), MIHOMO自定义优选单源上限);
            }));
        ...
        console.error(`读取mihomo自定义优选源失败: ${error.message}`);
    }
...
async function 拉取Mihomo优选源() {
            ...
            if (source.parser === 'api') {
                ...
                try {
                    const response = await fetch(source.url, {
                        ...
                        headers: { 'User-Agent': 'cf-worker-mihomo provider' }
                    });
                    if (!response.ok) throw new Error(`HTTP ${response.status}`);
                    results.push(...解析Mihomo优选文本(await response.text(), source.name));
                } finally {
                    clearTimeout(timeoutId);
                }
            }
        } catch (error) {
            console.error(`拉取mihomo优选源失败 ${source.name}: ${error.message}`);
        }
    ...
    return 去重Mihomo优选IP(settled.flatMap(result => result.status === 'fulfilled' ? result.value : []));
}

function 解析Mihomo优选文本(text, sourceName) {
    const results = [];
    const 原文 = String(text || '');
    const 文本列表 = [原文];
    if (isValidBase64(原文)) {
        try { 文本列表.push(base64Decode(原文)); } catch { }
    }
    for (const 当前文本 of 文本列表) {
        const lines = 当前文本.replace(/\r/g, '\n').split('\n');
        for (const line of lines) {
            for (const rawToken of line.split(/[,\s]+/)) {
                const token = rawToken.trim();
                if (!token) continue;
                const parsed = 标准化Mihomo优选IP(token, sourceName);
                if (parsed) results.push(parsed);
            }
        }
        const ipv4Matches = 当前文本.match(/\b(?:25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(?:\.(?:25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}\b/g) || [];
        for (const ip of ipv4Matches) {
            const parsed = 标准化Mihomo优选IP(ip, sourceName);
            if (parsed) results.push(parsed);
        }
    }
    return results;
...
async function 解析Mihomo优选域名(source) {
    const results = [];
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 3000);
    try {
        const dohURL = `https://cloudflare-dns.com/dns-query?name=${encodeURIComponent(source.hostname)}&type=A`;
        const response = await fetch(dohURL, {
            signal: controller.signal,
            headers: {
                'Accept': 'application/dns-json',
                'User-Agent': 'cf-worker-mihomo provider'
            }
        });
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        ...
        for (const answer of data.Answer || []) {
            ...
            const parsed = 标准化Mihomo优选IP(answer.data, source.name);
            if (parsed) results.push(parsed);
        }
    } catch (error) {
        console.error(`解析mihomo优选域名失败 ${source.hostname}: ${error.message}`);
    } finally {
        clearTimeout(timeoutId);
    }
    return results;
...
function 是有效IPv4(address) {
    if (/^(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}$/.test(address)) return !是保留IPv4(address);
    return false;
}

function 是保留IPv4(address) {
...
function 限制Mihomo数量(list, limit) {
    const clean = Array.isArray(list) ? list.filter(Boolean) : [];
    return limit ? clean.slice(0, limit) : clean;
}

function 生成MihomoProvider(地址列表, config_JSON, host) {
    const yamlQuote = value => JSON.stringify(String(value));
    const 节点路径 = config_JSON.启用0RTT ? config_JSON.PATH + '?ed=2560' : config_JSON.PATH;
    const 节点Host = (config_JSON.HOSTS && config_JSON.HOSTS[0]) || host;
    const fingerprint = config_JSON.Fingerprint || 'chrome';
    const skipCertVerify = config_JSON.跳过证书验证 ? 'true' : 'false';
    const lines = ['proxies:'];
    let index = 0;

    for (const 原始地址 of 地址列表) {
        const match = String(原始地址).match(/^(\[[\da-fA-F:]+\]|[\d.]+|[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)*)(?::(\d+))?(?:#(.+))?$/);
        if (!match) continue;
        index += 1;
        const server = match[1].replace(/^\[/, '').replace(/\]$/, '');
        const port = match[2] || '443';
        const remark = decodeURIComponent(match[3] || server).replace(/[\r\n]/g, ' ').trim();
        const name = `ACCB-CF-${String(index).padStart(4, '0')}-${remark}`;
        ...
        lines.push(
            ...
            `    port: ${port}`,
            `    uuid: ${yamlQuote(config_JSON.UUID)}`,
            '    tls: true',
            '    udp: true',
            `    servername: ${yamlQuote(节点Host)}`,
            '    network: ws',
            `    client-fingerprint: ${yamlQuote(fingerprint)}`,
            `    skip-cert-verify: ${skipCertVerify}`,
            '    ws-opts:',
            `      path: ${yamlQuote(节点路径)}`,
            '      headers:',
            `        Host: ${yamlQuote(节点Host)}`
        );
    }
...
function 生成MihomoDebug统计(地址列表) {
    ...
    for (const item of 地址列表) {
        ...
        const category = remark === 'CF-CIDR-random' ? 'random' :
            remark === 'ADD.txt' ? 'custom' :
            remark === 'ADDAPI' ? 'custom-api' :
            remark.startsWith('domain-') ? 'doh-domain' :
            remark === 'api-source' ? 'api' :
            'page-or-text';
        stats.categories[category] = (stats.categories[category] || 0) + 1;
        stats.remarks[remark] = (stats.remarks[remark] || 0) + 1;
        if (category === 'random') stats.random += 1;
        else stats.preferred += 1;
    }
    return stats;
}
    ...
    await Promise.allSettled(urls.map(async (url) => {
        try {
            ...
            */
            ...
            const lines = 预处理订阅明文内容.trim().split('\n').map(l => l.trim()).filter(l => l);
            ...
            if (!isCSV) {
                ...
                } else if (headers.some(h => /ip/i.test(h))) {
                    const ipIdx = headers.findIndex(h => /ip/i.test(h));
                    const portIdx = headers.findIndex(h => /port|端口/i.test(h));
                    const tlsIdx = headers.findIndex(h => /^tls$/i.test(h));
                    const delayIdx = headers.findIndex(h => /delay|latency|ping|延迟/i.test(h));
                    const speedIdx = headers.findIndex(h => /speed|download|下载/i.test(h));
                    dataLines.forEach(line => {
                        const cols = line.split(',').map(c => c.trim());
                        if (!cols[ipIdx]) return;
                        if (tlsIdx !== -1 && !/^(true|1|yes)$/i.test(cols[tlsIdx])) return;
                        const wrappedIP = IPV6_PATTERN.test(cols[ipIdx]) ? `[${cols[ipIdx]}]` : cols[ipIdx];
                        const port = portIdx !== -1 && cols[portIdx] ? cols[portIdx] : (new URL(url).searchParams.get('port') || 默认端口);
                        const remark = speedIdx !== -1 ? `#CF优选 ${cols[delayIdx] || ''}ms ${cols[speedIdx]}` : '#CF优选';
